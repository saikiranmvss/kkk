...
$selectProds=$dbConn->execute("SELECT prod_id,prod_name,prod_primary_color , prod_slab_size1 , prod_slab_size2 ,prod_stone_cat , prod_finishes ,prod_images From products_data WHERE prod_images IS NOT NULL AND prod_status!=1 ORDER BY created_date DESC limit 18");
...
$selectSizesFilter=$dbConn->execute("SELECT GROUP_CONCAT(DISTINCT CONCAT(prod_slab_size1, ',', prod_slab_size2) SEPARATOR ',' ) as sizes From products_data WHERE prod_images IS NOT NULL AND prod_status!=1 AND prod_slab_size1!=' ' AND prod_slab_size2!=' ' AND prod_slab_size1!='.' AND prod_slab_size2!='.' ORDER BY created_date DESC");
...
<!-- Google tag (gtag.js) -->
 <script async src="https://www.googletagmanager.com/gtag/js?id=G-TQW783CZ99"></script>
<script> window.dataLayer = window.dataLayer || []; function gtag(){dataLayer.push(arguments);} gtag('js', new Date()); gtag('config', 'G-TQW783CZ99'); </script>
...
<!--                            <a class="link">Join Franchise</a>-->
        ...
        <div class="prod-gallery" id="prod-gallery">
           ...
           <?php while($selectProdsRow=mysqli_fetch_array($selectProds)){ ?>
            ...
            <a href="product-view.php?data=<?=$selectProdsRow['prod_id']?>" prod-name="<?=strtolower($selectProdsRow['prod_name'])?>" class="wrap" data-color="<?=$selectProdsRow['prod_primary_color']?>" data-finish="<?=implode(',',json_decode($selectProdsRow['prod_finishes']))?>" data-cat="<?=$selectProdsRow['prod_stone_cat']?>" data-size1="<?=$selectProdsRow['prod_slab_size1']?>" data-size2="<?=$selectProdsRow['prod_slab_size2']?>" >
                       ...
                       <div class="foot-text">
                           <img src="images/logo-white.png" class="footer-logo">
                            ...
                            <div class="footer-social-links">
                                <a href="https://www.facebook.com/agastyastones/"><i class="fab fa-facebook-f"></i></a>
                                <a href="https://www.instagram.com/agastyastones/"><i class="fab fa-instagram"></i></a>
                                <a href="https://twitter.com/AgastyaStones"><i class="fab fa-x-twitter"></i></a>
                                <a href="https://www.linkedin.com/company/agstones/"><i class="fab fa-linkedin"></i></a>
                            </div>
...
if(string==''){
    ...
    // $('.prod-gallery a[prod-name*="'+string+'"]').show();
